...
var map_data: Array[Array] # Matrix
...
func _generate_initial_grid() -> Array[Array]:
	...
	for i in FLOORS:
		...
		for j in COLUMNS:
			...
			if i == FLOORS - 1:
				current_room.position.y = (i + 1) * -Y_DIST # + 1 to add distance between last node and boss room
...
func _setup_connection(i: int, j: int) -> int:
	...
	while not next_room or _would_cross_existing_path(i, j, next_room):
		var random_j := clampi(randi_range(j - 1, j + 1), 0, COLUMNS - 1) # Clamp to make sure we don't generate an index outside of the array
